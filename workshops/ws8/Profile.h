...
struct Profile {
    ...
    void validateAddress() const {
        if (m_address.number < 0) {
            throw std::runtime_error("Error: Address number is a negative.");
        }
        if (m_address.postal_code.length() == Address_postal_code_length) {
            if (!(isalpha(m_address.postal_code[0]) && isdigit(m_address.postal_code[1]) && isalpha(m_address.postal_code[2]) &&
                  isspace(m_address.postal_code[3]) && isdigit(m_address.postal_code[4]) && isalpha(m_address.postal_code[5]) &&
                  isdigit(m_address.postal_code[6]))) {
                throw std::runtime_error("Error: Postal Code character is incorrect");
            }
        } else {
            throw std::runtime_error("Error: Postal Code is not the correct length");
        }
    }
